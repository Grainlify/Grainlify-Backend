...
// TestAuthorizeURL_StatePassthrough asserts that the state parameter is included
// verbatim in the generated authorization URL so the callback can validate CSRF.
// This is the critical invariant: whatever state is generated by the caller must
// round-trip through GitHub untouched.
func TestAuthorizeURL_StatePassthrough(t *testing.T) {
	cases := []struct {
		name  string
		state string
	}{
		{"random 32-byte base64", "dGVzdC1zdGF0ZS12YWx1ZS0xMjM0NTY"},
		{"state with pipe separator (encoded)", "dGVzdHxodHRwczovL2V4YW1wbGUuY29t"},
		{"empty state (rejected by handler, but URL builds)", ""},
		{"url-safe base64 characters", "abc-def_ghi"},
	}

	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			rawURL, err := AuthorizeURL("client123", "https://example.com/cb", tc.state, nil)
			if err != nil {
				t.Fatalf("AuthorizeURL() unexpected error: %v", err)
			}
			...
			if gotState != tc.state {
				t.Errorf("state round-trip failed: sent %q, URL contains %q", tc.state, gotState)
			}
		})
	}
}

// TestAuthorizeURL_StateRequired documents that an empty state is a security
// misconfiguration: the generated URL has no CSRF protection.  AuthorizeURL
// itself does not reject an empty state (callers own that responsibility), but
// this test records the current behavior and makes the gap visible.
func TestAuthorizeURL_EmptyStateIsInsecure(t *testing.T) {
	rawURL, err := AuthorizeURL("client123", "https://example.com/cb", "", nil)
	if err != nil {
		t.Fatalf("AuthorizeURL() unexpected error: %v", err)
	}
	parsed, _ := url.Parse(rawURL)
	gotState := parsed.Query().Get("state")
	// Document (not fix) the gap: AuthorizeURL allows empty state.
	// The handler layer (LoginStart / Start) is responsible for always supplying
	// a non-empty cryptographically random state.
	if gotState != "" {
		t.Errorf("expected empty state to pass through as-is, got %q", gotState)
	}
	// Callers must never pass an empty state in production.
	t.Log("SECURITY NOTE: empty state parameter provides no CSRF protection; " +
		"handlers/github_oauth.go is responsible for supplying a random state")
}
